...
if [ "$FIX_MODE" = true ]; then
    black ic_python_db tests
else
    black ic_python_db tests --check
fi

# Check/fix imports with isort
echo "Running isort..."
if [ "$FIX_MODE" = true ]; then
    isort ic_python_db tests
else
    isort ic_python_db tests --check-only
fi

# Lint with flake8 (no auto-fix available)
echo "Running flake8..."
# Using configuration from setup.cfg
flake8 ic_python_db tests --exclude=tests/venv,tests/.basilisk,*/.basilisk/*,*/__pycache__/*

# Type check with mypy (no auto-fix available)
echo "Running mypy..."
# Using configuration from setup.cfg
mypy ic_python_db tests
